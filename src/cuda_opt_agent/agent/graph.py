"""
LangGraph 状态机 —— 编排整个迭代优化流程。

状态转移:
  [*] → init → bootstrap → compile_validate → compare_library → profile_best → analyze
  analyze → decide
  decide → hp_search (含超参) / apply_direct (无超参) / terminate
  hp_search → evaluate
  apply_direct → evaluate
  evaluate → reflect
  reflect → terminate (停止) / profile_best (继续)
  terminate → [*]
"""
from __future__ import annotations
import asyncio
import logging
import warnings
from typing import Any

from langchain_core._api.deprecation import LangChainPendingDeprecationWarning
warnings.filterwarnings("ignore", message=r"The default value of `allowed_objects`",
                         category=LangChainPendingDeprecationWarning)

from langgraph.graph import END, StateGraph

from ..config import load_config
from ..memory.knowledge import KnowledgeBase
from ..memory.run_state import RunStateManager
from ..models.data import AgentConfig, OperatorSpec, RunState
from .llm_client import LLMClient, LLMStreamSink
from .nodes import AgentNodes
from .state import GraphState

logger = logging.getLogger(__name__)


def build_graph(config: AgentConfig | None = None, state_manager: RunStateManager | None = None,
                kb: KnowledgeBase | None = None, llm: LLMClient | None = None,
                entry_point: str = "init", stream_sink: LLMStreamSink = None) -> StateGraph:
    if config is None:
        config = load_config()
    if state_manager is None:
        state_manager = RunStateManager(config)
    if kb is None:
        kb = KnowledgeBase(config.knowledge_base_dir)
    if llm is None:
        llm = LLMClient(
            provider=config.llm_provider,
            model=config.llm_model,
            stream_sink=stream_sink,
            use_tool_use=config.use_tool_use,
        )
    elif stream_sink is not None:
        llm.stream_sink = stream_sink
    if hasattr(llm, "use_tool_use"):
        llm.use_tool_use = config.use_tool_use

    nodes = AgentNodes(state_manager=state_manager, kb=kb, llm=llm,
                        stream_sink=stream_sink, config=config)

    graph = StateGraph(GraphState)
    graph.add_node("init", nodes.init_node)
    graph.add_node("bootstrap", nodes.bootstrap_node)
    graph.add_node("compile_validate", nodes.compile_and_validate_node)
    graph.add_node("compare_library", nodes.compare_library_node)
    graph.add_node("profile_best", nodes.profile_best_node)
    graph.add_node("analyze", nodes.analyze_node)
    graph.add_node("decide", nodes.decide_node)
    graph.add_node("hp_search", nodes.hp_search_node)
    graph.add_node("apply_direct", nodes.apply_direct_node)
    graph.add_node("evaluate", nodes.evaluate_node)
    graph.add_node("reflect", nodes.reflect_node)
    graph.add_node("terminate", nodes.terminate_node)

    graph.set_entry_point(entry_point)
    graph.add_edge("init", "bootstrap")
    graph.add_edge("bootstrap", "compile_validate")
    graph.add_conditional_edges("compile_validate", _route_after_compile,
                                 {"compare_library": "compare_library", "terminate": "terminate"})
    graph.add_edge("compare_library", "profile_best")
    graph.add_edge("profile_best", "analyze")
    graph.add_edge("analyze", "decide")
    graph.add_conditional_edges("decide", _route_after_decide,
                                 {"hp_search": "hp_search", "apply_direct": "apply_direct", "terminate": "terminate"})
    graph.add_edge("hp_search", "evaluate")
    graph.add_edge("apply_direct", "evaluate")
    graph.add_edge("evaluate", "reflect")
    graph.add_conditional_edges("reflect", _route_after_reflect,
                                 {"profile_best": "profile_best", "terminate": "terminate"})
    graph.add_edge("terminate", END)
    return graph


def _route_after_compile(state: GraphState) -> str:
    if state.get("trial_compile_ok") and state.get("trial_correctness_ok"):
        return "compare_library"
    return "terminate"

def _route_after_decide(state: GraphState) -> str:
    if state.get("should_stop"):
        return "terminate"
    return "hp_search" if state.get("has_hyperparams") else "apply_direct"

def _route_after_reflect(state: GraphState) -> str:
    return "terminate" if state.get("should_stop") else "profile_best"


def run_optimization(operator_spec: OperatorSpec, config: AgentConfig | None = None,
                     resume_dir: str | None = None,
                     stream_sink: LLMStreamSink = None) -> RunState:
    try:
        asyncio.get_running_loop()
    except RuntimeError:
        return asyncio.run(run_optimization_async(operator_spec, config=config,
                                                    resume_dir=resume_dir, stream_sink=stream_sink))
    raise RuntimeError("Use run_optimization_async() from async context.")


async def run_optimization_async(
    operator_spec: OperatorSpec, config: AgentConfig | None = None,
    resume_dir: str | None = None, stream_sink: LLMStreamSink = None,
) -> RunState:
    if config is None:
        config = load_config()
    sm = RunStateManager(config)
    kb = KnowledgeBase(config.knowledge_base_dir)
    llm = LLMClient(
        provider=config.llm_provider,
        model=config.llm_model,
        stream_sink=stream_sink,
        use_tool_use=config.use_tool_use,
    )

    if resume_dir:
        run_state = sm.resume_run(run_dir=resume_dir)
        if run_state is None:
            raise FileNotFoundError(f"Cannot resume from {resume_dir}")
        entry_point = "profile_best" if run_state.iterations else "init"
    else:
        run_state = sm.new_run(operator_spec)
        entry_point = "init"

    graph = build_graph(config=config, state_manager=sm, kb=kb, llm=llm,
                         entry_point=entry_point, stream_sink=stream_sink)
    compiled = graph.compile()

    initial: dict[str, Any] = {
        "operator_spec": run_state.operator_spec if resume_dir else operator_spec,
        "hardware_spec": run_state.hardware_spec,
        "run_state": run_state, "config": config,
        "should_stop": False, "iteration_count": len(run_state.iterations),
        "blacklist": [], "external_knowledge": None, "effective_methods_list": [],
        "consecutive_rejects": 0, "max_iterations": config.max_iterations,
    }

    await compiled.ainvoke(initial)
    sm.mark_done()
    return sm.state
