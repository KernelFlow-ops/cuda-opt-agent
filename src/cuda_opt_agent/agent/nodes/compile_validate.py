    ...
    for attempt in range(max_retries + 1):
        ...
        if cr.success:
            ...
            if attempt < max_retries:
                code = await self._repair_code(code, f"Correctness failed: {message}", hw)
            ...
            if attempt < max_retries:
                code = await self._repair_code(code, compile_output, hw)
                code_path = await asyncio.to_thread(
                    self.sm.persistence.save_code,
                    code,
                    iter_dir,
...
async def _repair_code(self, code: str, error: str, hw) -> str:
    """LLM 修复编译/校验错误。"""
